...
class JdbcTemplateMovieRepository implements MovieRepository {
   ...
   //     KeyHolder keyHolder = new GeneratedKeyHolder();
        jdbcTemplate.update("INSERT INTO movie VALUES (?, ?, ?, ?)",
                new Object[]{movie.getId().getId(), movie.getTitle(), movie.getType().toString(), movie.getYear().getValue()}
 //               keyHolder
                );
//        keyHolder.getKey().toString()
    ...
    public Collection<Movie> findAllBefore(int year) {
        return jdbcTemplate.query("SELECT * FROM movie WHERE year < ?"
            , new Object[]{year}
            , new MovieRowMapper()
        );
    }
    ...
    public Collection<Movie> findByYearBetween(int yearStart, int yearEnd) {
        return jdbcTemplate.query("SELECT * FROM movie WHERE year BETWEEN ? AND year ?"
